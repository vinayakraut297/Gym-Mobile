...
import { LogOut, Coins, Moon, Bell, Shield } from 'lucide-react-native';
...
export default function OwnerSettingsScreen() {
  ...
  const [gymName, setGymName] = useState(user?.branch || 'FitPulse Elite');
  const [phone, setPhone] = useState(user?.phone || '+91 98765 43210');

  const handleSave = () => {
    updateUser({ branch: gymName, phone });
    Alert.alert('Settings Saved', 'Business configuration records updated.');
  };
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]} edges={['top', 'left', 'right']}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.header}>
          ...
          <Text style={[styles.subtitle, { color: colors.textSecondary }]}>Manage your facility settings and tokens.</Text>
        </View>
        ...
        <View style={styles.section}>
          ...
          <Card style={{ marginVertical: 0 }}>
            ...
            <Button title="Save Settings" onPress={handleSave} style={{ marginTop: 16 }} />
          </Card>
        </View>

        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: colors.textSecondary }]}>AI BUSINESS COINS</Text>
          <Card style={{ marginVertical: 0 }}>
        ...
        {/* Preferences Section */}
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { color: colors.textSecondary }]}>SYSTEM PREFERENCES</Text>
